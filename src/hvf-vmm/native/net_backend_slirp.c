...
static int64_t now(void *opaque){(void)opaque;struct timespec t;clock_gettime(CLOCK_MONOTONIC,&t);return (int64_t)t.tv_sec*1000000000+t.tv_nsec;}
...
int net_backend_open(const struct hvf_options *options,net_receive_fn receive){
    ...
    for(unsigned i=0;i<options->forward_count;i++){
        ...
        if(slirp_add_hostfwd(slirp,f->udp,host,f->host_port,guest,f->guest_port))return -1;
    }
    return 0;
}
void net_backend_send(const void *data,size_t size){slirp_input(slirp,data,(int)size);}
void net_backend_close(void){if(slirp){slirp_cleanup(slirp);slirp=NULL;}}
